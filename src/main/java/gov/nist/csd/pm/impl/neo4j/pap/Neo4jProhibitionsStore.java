...
import static gov.nist.csd.pm.impl.neo4j.pap.Neo4jGraphStore.*;

public class Neo4jProhibitionsStore implements ProhibitionsStore {
    ...
    public Neo4jProhibitionsStore(Neo4jConnection neo4j) {
        this.neo4j = neo4j;
    }
    ...
    public void update(String name, ProhibitionSubject subject, AccessRightSet accessRights, boolean intersection,
        ...
        try {
            neo4j.runTx(tx -> {
                checkUpdateInput(new Neo4jGraphStore(neo4j), name, subject, accessRights, intersection, containerConditions);
