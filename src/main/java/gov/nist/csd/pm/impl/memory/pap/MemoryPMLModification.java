...
class MemoryPMLModification extends MemoryStore<TxPMLModification> implements PMLModification, Transactional, BaseMemoryTx {

    private final Map<String, FunctionDefinitionStatement> functions;
    private final Map<String, Value> constants;
    private MemoryGraphModification graph;

    public MemoryPMLModification() {
        this.functions = new HashMap<>();
        this.constants = new HashMap<>();
    }

    public MemoryPMLModification(PMLModification PMLModification) throws PMException {
        this.functions = PMLModification.getFunctions();
        this.constants = PMLModification.getConstants();
    }

    public void setMemoryGraph(MemoryGraphModification graph) {
        this.graph = graph;
    }
    ...
    public void beginTx() {
        if (tx == null) {
            tx = new MemoryTx<>(false, 0, new TxPMLModification(new TxOpTracker(), this));
        }
        tx.beginTx();
    }
