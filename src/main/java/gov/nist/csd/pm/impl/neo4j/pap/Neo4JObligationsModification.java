...
import static gov.nist.csd.pm.impl.neo4j.pap.Neo4JGraphModification.NAME_PROPERTY;

public class Neo4JObligationsModification implements ObligationsModification {
    ...
    public Neo4JObligationsModification(Neo4jConnection neo4j) {
        this.neo4j = neo4j;
    }
    ...
    public void create(UserContext author, String name, Rule... rules)
        ...
        try {
            neo4j.runTx(tx -> {
                checkCreateInput(new Neo4JGraphModification(neo4j), author, name, rules);
    ...
    public void update(UserContext author, String name, Rule... rules)
        ...
        try {
            neo4j.runTx(tx -> {
                checkUpdateInput(new Neo4JGraphModification(neo4j), author, name, rules);
