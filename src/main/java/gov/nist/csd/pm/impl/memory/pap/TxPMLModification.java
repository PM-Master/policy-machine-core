...
public class TxPMLModification implements PMLModification, BaseMemoryTx {

    private final TxOpTracker txOpTracker;
    private final MemoryPMLModification memoryUserDefinedPMLStore;

    public TxPMLModification(TxOpTracker txOpTracker, MemoryPMLModification memoryUserDefinedPMLStore) {
    ...
    public void rollback() {
        ...
        for (Operation event : events) {
            try {
                TxCmd<MemoryPMLModification> txCmd = (TxCmd<MemoryPMLModification>) TxCmd.eventToCmd(event);
