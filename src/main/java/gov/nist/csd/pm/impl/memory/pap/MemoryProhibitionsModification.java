...
class MemoryProhibitionsModification extends MemoryStore<TxProhibitionsModification> implements ProhibitionsModification, Transactional, BaseMemoryTx {

    private Map<String, List<Prohibition>> prohibitions;
    private MemoryGraphModification graph;

    public MemoryProhibitionsModification() {
    ...
    public void beginTx() {
        if (tx == null) {
            tx = new MemoryTx<>(false, 0, new TxProhibitionsModification(new TxOpTracker(), this));
        }
